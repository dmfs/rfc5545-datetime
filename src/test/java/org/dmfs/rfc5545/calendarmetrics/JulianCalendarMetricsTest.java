...
public class JulianCalendarMetricsTest
{
    ...
    public void testGetMaxMonthDays()
    {
        for (int minDaysInFirstWeek = 1; minDaysInFirstWeek < 8; ++minDaysInFirstWeek)
        {
            ...
            {
                CalendarMetrics tools = new JulianCalendarMetrics(Weekday.values()[weekStart], minDaysInFirstWeek);
    ...
    public void testGetMonthAndDayOfYearDay()
    {
        ...
        for (int minDaysInFirstWeek = 1; minDaysInFirstWeek < 8; ++minDaysInFirstWeek)
        {
            ...
            for (int weekStart = 0; weekStart < 7; ++weekStart)
            {
                CalendarMetrics tools = new JulianCalendarMetrics(Weekday.values()[weekStart], minDaysInFirstWeek);
                testCal.setFirstDayOfWeek(weekStart + 1);
                for (int year = MIN_YEAR; year < 2035; ++year)
                {
    ...
    public void testGetUtcTimeStamp()
    {
        ...
        for (int minDaysInFirstWeek = 1; minDaysInFirstWeek < 8; ++minDaysInFirstWeek)
        {
            ...
            for (int weekStart = 0; weekStart < 7; ++weekStart)
            {
                JulianCalendarMetrics tools = new JulianCalendarMetrics(Weekday.values()[weekStart], minDaysInFirstWeek);
                testCal.setFirstDayOfWeek(weekStart + 1);
                for (int year = MIN_YEAR; year < MAX_YEAR; ++year)
                {
                    testCal.set(year, 0, 1);
    ...
    public void testGetTimeStamp()
    {
                ...
                "Australia/Sidney" })
        {
            ...
            CalendarMetrics tools = new JulianCalendarMetrics(Weekday.MO, 4);

            for (int year = MIN_YEAR; year < MAX_YEAR; ++year) /* there seems to be some issue when we test dates after 2038, maybe a year 2038 bug somewhere? */
            {
                String errMsg = "";
                for (int month = 0; month < tools.getMonthsPerYear(year); ++month)
                {
